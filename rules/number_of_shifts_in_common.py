def get(_rule, model, shifts, _allocations, uuid_func):
    ...
    number_of_hours = _rule['params']['numberOfShifts']

    overlaps = []
    for i in range(0, len(shifts)):
        for j in range(i, len(shifts)):
            are_overlapping = 1 if max(0, min(shifts[i]['end'], shifts[j]['end']) - max(shifts[i]['start'], shifts[j]['start'])) > 0 else 0
            ...
            overlaps.append(are_overlapping * are_staffed_on_overlapping_shifts)
